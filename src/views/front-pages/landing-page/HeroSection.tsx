...
const HeroSection = ({ mode }: { mode: SystemMode }) => {
  ...
  return (
    <section id='home' className='overflow-hidden pbs-[75px] -mbs-[75px] relative'>
      <img
        src={heroSectionBg}
        alt={t('dms.frontPages.landing.hero.backgroundAlt')}
      ...
      <div className={classnames('pbs-[88px] overflow-hidden', frontCommonStyles.layoutSpacing)}>
        <div className='md:max-is-[550px] mbs-0 mbe-7 mli-auto text-center relative'>
          ...
          >
            {t('dms.frontPages.landing.hero.title')}
          </Typography>
          <Typography className='font-medium' color='text.primary'>
            {t('dms.frontPages.landing.hero.subtitle')}
          </Typography>
          <div className='flex mbs-6 items-baseline justify-center relative'>
            <div className='flex gap-2 absolute inline-start-[0%] block-start-[41%] max-md:hidden'>
              <Typography className='font-medium'>{t('dms.frontPages.landing.hero.joinCommunity')}</Typography>
              <img
                src='/images/front-pages/landing-page/join-community-arrow.png'
                alt={t('dms.frontPages.landing.hero.joinCommunityArrowAlt')}
                height='48'
                width='60'
              />
            </div>
            <Button
              ...
              color='primary'
            >
              {t('dms.frontPages.landing.hero.getEarlyAccess')}
            </Button>
          </div>
        </div>
      </div>
      ...
      >
        <Link href='/' target='_blank' className='block relative'>
          <img
            src={dashboardImage}
            alt={t('dms.frontPages.landing.hero.dashboardAlt')}
            className={classnames('mli-auto', styles.heroSecDashboard)}
          />
          <div className={classnames('absolute', styles.heroSectionElements)}>
            <img src={elementsImage} alt={t('dms.frontPages.landing.hero.elementsAlt')} />
          </div>
        </Link>
      </div>
    </section>
