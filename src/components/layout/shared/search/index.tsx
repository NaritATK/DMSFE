...
const CommandFooter = ({ t }: { t: (key: string, fallback?: string) => string }) => {
  return (
    <div cmdk-footer=''>
      <div className='flex items-center gap-1'>
        ...
        <span>{t('dms.search.footer.toNavigate')}</span>
      </div>
      <div className='flex items-center gap-1'>
        <kbd>
          <i className='tabler-corner-down-left text-base' />
        </kbd>
        <span>{t('dms.search.footer.toOpen')}</span>
      </div>
      <div className='flex items-center gap-1'>
        <kbd>esc</kbd>
        <span>{t('dms.search.footer.toClose')}</span>
      </div>
    </div>
  )
}
    ...
    <>
      {isBreakpointReached || settings.layout === 'horizontal' ? (
        ...
        <div className='flex items-center gap-2 cursor-pointer' onClick={() => setOpen(true)}>
          ...
          <div className='whitespace-nowrap select-none text-textDisabled'>{t('dms.search.trigger')} ⌘K</div>
        </div>
      )}
      <CommandDialog open={open} onOpenChange={setOpen}>
        <div className='flex items-center justify-between border-be pli-4 plb-3 gap-2'>
          ...
          <CommandInput
            ...
            placeholder={t('dms.search.inputPlaceholder')}
          />
          <span className='text-textDisabled'>[{t('dms.search.escKey')}]</span>
